...
export function buildProgramAtomMirrorLine(personaId, atom) {
  const { name } = getAtomTeachingCopy(atom)
  if (personaId === 'vasily') {
    return `Засчитано. Встань перед зеркалом и проживи «${name}» в теле — зону смотрел в «Как учить». Без зеркала кинестетика пустая.`
  }
  if (personaId === 'arkady') {
    return `Верно, друг. Попробуй «${name}» перед зеркалом — картинку зоны помнишь из «Как учить»? Прочувствуй в мышцах.`
  }
  return `Принято. Попробуй «${name}» перед зеркалом. Подсказка по зоне — в «Как учить».`
}
