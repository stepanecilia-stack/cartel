...
export const TECHNIQUE_LEVEL2_ATOMS = DEFAULT_TECHNICAL_LEVEL2

export { getDefaultTechnicalProgramAtoms }
...
export const getWeights = (studentData = {}) => {
  ...
  if (wf.tacticMode === 'infighter') {
    ...
    return {
      ...
      apeIndex,
      tacticAdvice:
        'Рост ниже ориентира по таблице для этой категории — упор на сокращение дистанции и агрессивное давление.',
...
export const loadLegacyTechnicalAtoms = async () => {
  const cached = getTechnicalProgramAtomsCache().level1
  if (cached.length > 0) return cached
  try {
    await loadTechnicalProgramAtomsOnce()
    const loaded = getTechnicalProgramAtomsCache().level1
    if (loaded.length > 0) return loaded
  } catch (err) {
    console.warn('loadLegacyTechnicalAtoms:', err)
  }
  return DEFAULT_TECHNICAL_LEVEL1
}
