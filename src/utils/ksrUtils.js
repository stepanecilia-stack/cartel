...
export const getWeights = (studentData = {}) => {
  ...
  if (wf.tacticMode === 'infighter') {
    ...
    return {
      ...
      apeIndex,
      tacticAdvice:
        'Критический дефицит роста. Работа только в ближнем бою через плотный блок и давление.',
  ...
  if (wf.tacticMode === 'outfighter') {
    ...
    return {
      ...
      apeIndex,
      tacticAdvice:
        'Максимальное преимущество в росте. Запрет на сближение. Работа только через джеб и дистанцию.',
      ...weightFirstBase(studentData, wf),
    }
  }
